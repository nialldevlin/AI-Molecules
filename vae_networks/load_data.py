...
class DataHandler:
    def __init__(self, filename, split, flatten=True, transpose=False):
        self.header = ""
        self.separator = "Direct configuration="
        self.precision = 8
            ...
            for line in f:
                if notHeader:
                    if line.startswith(self.separator):
                        data.append(section)
                        section = []  # Reset the section for the new one
                    else:
                        section.append([float(x) for x in line.split()])
                elif line.startswith(self.separator):
                    notHeader = True  # Set the flag to indicate header is over
                elif not notHeader:
                    self.header += line  # Create header for writing files
            data.append(section)
